...
class GarminSDDataHandler {
  ...
  function initialize(versionStr as String) {
    ...
    // On Start-up we show the app version number in place of status.
  ...
  // Prints accelerometer data that is received from the system
  ...
  function onStart() as Void {
    ...
    // ---- ADDITION: simulate test data if running in simulator ----
    var devSettings = System.getDeviceSettings();
    if (devSettings.partNumber == "SIMULATOR") {
        writeLog(tagStr, "Simulator detected - loading test data");
        loadTestData();  // call simulated data generator
    }
  }
  ...
  /////////////////////////////
  // ---- ADDED TEST FUNCTION ----
  function loadTestData() as Void {
    /*
      Populates mSamplesX/Y/Z with fake data to test the app in the simulator.
      This creates a simple oscillating pattern similar to motion data.
    */
    var totalSamples = ANALYSIS_PERIOD * SAMPLE_FREQUENCY;
    for (var i = 0; i < totalSamples; i += 1) {
      var t = i / SAMPLE_FREQUENCY;
      // Simulated motion data (sine waves)
      mSamplesX[i] = 0.2 * Math.sin(2 * Math.PI * 1 * t);
      mSamplesY[i] = 0.2 * Math.sin(2 * Math.PI * 1 * t + Math.PI / 2);
      mSamplesZ[i] = 0.9 + 0.1 * Math.sin(2 * Math.PI * 0.5 * t); // baseline gravity + variation
    }
    ...
    writeLog("loadTestData()", "Loaded " + totalSamples + " simulated samples");
